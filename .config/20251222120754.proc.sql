...
PROC: BEGIN
	...
	CASE inAction
		...
		WHEN 1 THEN
            ...
            SELECT userId INTO dUserId 
            FROM users 
            WHERE phone = inPhone OR email = inEmail 
            LIMIT 1;
            
			IF NOT ISNULL(dUserId) THEN
				SELECT 'The user already exists' message;
				LEAVE PROC;
            END IF;
            ...
            START TRANSACTION;
				...
				INSERT INTO users(email, phone, fname, lname, active, pass)
				VALUES(inEmail, inPhone, inFname, inLname, 1, inPass);
				
				SET dUserId = LAST_INSERT_ID();
				
				INSERT INTO usersroles(roleId, adminId, userId, active)
				VALUES(inRoleId, inAdminId, dUserId, 1);
                
			IF dRollBack THEN
				ROLLBACK;
				SELECT 'There is a technical problem' message;
			ELSE 
				COMMIT;	
				SELECT dUserId AS userId, 1 AS created, title FROM adminroles WHERE roleId = inRoleId;
			END IF;        
            LEAVE PROC;
		...
		WHEN 2 THEN
			...
			UPDATE users 
				SET 
					email = COALESCE(NULLIF(inEmail, ''), email),
					phone = COALESCE(NULLIF(inPhone, ''), phone),
					fname = COALESCE(NULLIF(inFname, ''), fname),
					lname = COALESCE(NULLIF(inLname, ''), lname),
					pass = COALESCE(NULLIF(inPass, ''), pass)
            WHERE
				userId = inUserId;
        ...
        WHEN 3 THEN
			UPDATE users SET active = inActive WHERE userId = inUserId;
            SELECT ROW_COUNT() updated;
			LEAVE PROC;
        ...
        -- separate the customer and user roles and permissions
		/*WHEN 5 THEN
...
-- Dump completed on 2025-12-22 12:07:54
